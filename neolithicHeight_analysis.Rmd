...
lm4 <- lm(nealeLDpred_prs ~ Region, data=neoOnly)
...
compiled <- statCalc(compiled, "femur_estimated", "Sex")
...
sexDim <- aggregate(stature_est~ Sex + Region, data=compiled, mean)
...
- PRS constructed with LDpred show Med individuals are shorter (p=`r round(summary(lm4)$coefficients[4,4], 4)`, beta=`r round(summary(lm4)$coefficients[4,1], 3)`, se=`r round(summary(lm4)$coefficients[4,2], 4)`)
...
- Nitrogen is generally elevated in males compared to females, but only significant in the Med (p = `r round(summary(lm7)$coefficients[8,4], 4)`, beta=`r round(summary(lm7)$coefficients[8,1], 3)`, se=`r round(summary(lm7)$coefficients[8,2], 4)`, bf=`r bf7@bayesFactor$bf`) **BF indicates that there are no inteaction effects, so differences in N are not systematically different between sexes and regions**
...
`r print(sexDim)`
- *Effect of PRS on femur length is just significant* (p=`r round(summary(lm10)$coefficients[3,4], 4)`; beta=`r summary(lm10)$coefficients[3,1]`, se=`r round(summary(lm10)$coefficients[3,2], 4)`, bf=`r bf10@bayesFactor$bf`) **BF**
...
- Males in both regions have qualitatively higher nitrogen, but interaction effect is not significant (p=`r round(summary(lm7)$coefficients[6,4], 4)`; beta=`r summary(lm7)$coefficients[6,1]`, se=`r round(summary(lm7)$coefficients[6,2], 4)`, bf=`r bf7@bayesFactor$bf`) **BF**
- 13C are the same between sexes (p=`r round(summary(lm6)$coefficients[6,4], 4)`; beta=`r summary(lm6)$coefficients[6,1]`, se=`r round(summary(lm6)$coefficients[6,2], 4)`, bf=`r bf6@bayesFactor$bf`) **BF**
- No statistical relationship between femur length and 15N (p=`r round(summary(lm11)$coefficients[4,4], 4)`; beta=`r summary(lm11)$coefficients[4,1]`, se=`r round(summary(lm11)$coefficients[4,2], 4)`, bf=`r bf11@bayesFactor$bf`) **BF** 
- No statistical relationship between femur length and 13C (p=`r round(summary(lm12)$coefficients[4,4], 4)`; beta=`r summary(lm12)$coefficients[4,1]`, se=`r round(summary(lm12)$coefficients[4,2], 4)`, bf=`r bf12@bayesFactor$bf`) **BF**
...
- No difference between incidence of LEH in males vs females (p=`r round(summary(glm1)$coefficients[3,4], 4)`; beta=`r summary(glm1)$coefficients[3,1]`, se=`r round(summary(glm1)$coefficients[3,2], 4)`, bf=`r bfg1@bayesFactor$bf`) **BF**
- Interaction effect between sex and LEH on femur length is not significant (p=`r round(summary(lm13)$coefficients[5,4], 4)`; beta=`r summary(lm13)$coefficients[5,1]`, se=`r round(summary(lm13)$coefficients[5,2], 4)`, bf=`r bf13@bayesFactor$bf`) **BF**
- Females with LEH have significantly shorter femora than females without (p=`r round(summary(lm14)$coefficients[2,4], 4)`; beta=`r summary(lm14)$coefficients[2,1]`, se=`r round(summary(lm14)$coefficients[2,2], 4)`, bf=`r bf14@bayesFactor$bf`) **BF**
- Males with LEH have the same femur length as those without (p=`r round(summary(lm15)$coefficients[2,4], 4)`; beta=`r summary(lm15)$coefficients[2,1]`, se=`r round(summary(lm15)$coefficients[2,2], 4)`, bf=`r bf15@bayesFactor$bf`) **BF**
...
- Cribra has no statistical relationship with femur length (p=`r round(summary(lm16)$coefficients[3,4], 4)`; beta=`r summary(lm16)$coefficients[3,1]`, se=`r round(summary(lm16)$coefficients[3,2], 4)`, bf=`r bf16@bayesFactor$bf`) **BF**
- Frequencies of porotic hyperostosis are not different between North and South (p=`r round(summary(glm3)$coefficients[4,4], 4)`; beta=`r summary(glm3)$coefficients[4,1]`, se=`r round(summary(glm3)$coefficients[4,2], 4)`, bf=`r bfg3@bayesFactor$bf`) **BF**
- Porotic hyperostosis is not related to femur length (p=`r round(summary(lm17)$coefficients[3,4], 4)`; beta=`r summary(lm17)$coefficients[3,1]`, se=`r round(summary(lm17)$coefficients[3,2], 4)`, bf=`r bf17@bayesFactor$bf`) **BF**
...
- *Proportions of WHG are weakly associated with femur length* (p=`r round(summary(lm20)$coefficients[3,4], 4)`; beta=`r summary(lm20)$coefficients[3,1]`, se=`r round(summary(lm20)$coefficients[3,2], 4)`, bf=`r bf20@bayesFactor$bf`) **BF**
...
ggplot(data=LEHplot[!is.na(LEHplot$Sex) & LEHplot$LEH != "",], aes(y=femur_estimated, x=Region, fill=LEH, shape=Sex, stroke=0)) +
...
lm41 <- lm(asMales_prs ~ Sex + Region, data=sexspecific)
lm42 <- lm(asFemales_prs ~ Sex + Region, data=sexspecific)
lm43 <- lm(sexspec_prs ~ Sex + Region, data=sexspecific)
lm44 <- lm(combo_prs ~ Sex + Region, data=sexspecific)
# lm44 <- lm(clumpImp ~ Region, data=sexspecific)
# lm45 <- lm(clumpImp_sibs ~ Region, data=sexspecific)
# lm46 <- lm(clumpUnImp ~ Sex * Region, data=sexspecific)
# lm47 <- lm(clumpUnImp_sibs ~ Sex * Region, data=sexspecific)
